...
print("\nThe Documentation of every function in Moildev SDK\n"
      "==================================================")
for name, _ in getmembers(Moildev, isfunction):
...
class TestMoildev(unittest.TestCase):
    ...
    def test_maps_panorama_car(self):
        maps_x, _ = self.moildev.maps_panorama_car(110, 140, -6, False)
    ...
    def test_maps_panorama_tube(self):
        maps_x, _ = self.moildev.maps_panorama_tube(20, 110, True)
        self.assertEqual(maps_x.all(), self.maps.all())
    ...
    def test_panorama_car(self):
        img = self.image.copy()
        image_anypoint_car = self.moildev.panorama_car(img, 1100, 0, 0, 0,1,0,1, True)
        self.assertEqual(image_anypoint_car.all(), img.all())

    def test_panorama_tube(self):
        img = self.image.copy()
        image_anypoint_car = self.moildev.panorama_tube(img, 20, 110, False)
        self.assertEqual(image_anypoint_car.all(), img.all())
